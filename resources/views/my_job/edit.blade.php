...
@section('masthead')
    <x-masthead-banner>
        <h2 class="font-montserrat font-semibold text-5xl text-white">Edit Job Posting</h2>
        <p class="font-normal text-base text-white">Update job details to keep your listing current and attract the right
            candidates.</p>
    </x-masthead-banner>
@endsection
...
@section('content')
    ...
    <div class="flex justify-center mb-[90px]">
        <x-card class="max-w-[800px] w-full">
            <form x-data="{
                tags: '{{ $job->tags }}'.split(',').filter(tag => tag.trim() !== ''),
                tagInput: ''
            }" @submit.prevent="$refs.jobPostFilters.submit()" x-ref="jobPostFilters"
                action="{{ route('my-job-listings.update', $job) }}" method="POST" class="w-full">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <x-text-input type="text" name="title" placeholder="Title" :value="$job->title"
                    ...
                    <div>
                        ...
                        <x-text-input type="text" name="location" placeholder="Location" :value="$job->location"
                    ...
                    <div class="col-span-2">
                        ...
                        <x-text-input type="textarea" name="description" placeholder="Description" :value="$job->description"
                    ...
                    <div>
                        ...
                        <x-text-input type="number" name="salary" placeholder="Salary" :value="$job->salary"
                    ...
                    <div>
                        ...
                        <x-radio-group name="category" :value="$job->category" :all-option="false"
                            :options="\App\Models\JobListing::$category"></x-radio-group>
                    </div>

                    <div>
                        <div class="mb-2 font-semibold text-sm">Job Type</div>
                        <x-radio-group name="type" :value="$job->type" :all-option="false"
                            :options="\App\Models\JobListing::$type"></x-radio-group>
                    </div>
                    ...
                    <div class="col-span-2">
                        <x-button type="submit" class="w-full h-12 !text-base">Update Job</x-button>
                    </div>
                </div>
            </form>
        </x-card>
