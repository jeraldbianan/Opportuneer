...
@section('hero')
    <x-hero>
        <figure class="relative h-auto max-h-full w-full max-w-[622px]">
            ...
            <figcaption
                ...
                <p class="font-open-sans text-3xl font-extrabold text-dark-blue" aria-label="Job Count">
                    {{ $jobs->count() }}
                </p>
        ...
        <header class="flex flex-col w-full max-w-[466px]">
            ...
            <form x-data="" x-ref="home-filters" action="{{ route('job-listings.index') }}" method="GET"
                aria-labelledby="jobSearchFormLabel">
                <label id="jobSearchFormLabel" class="sr-only">Job search form</label>
                <div class="mt-12 flex gap-2">
                    ...
                    <x-text-input icon="location" placeholder="Location" name="location" value="{{ request('location') }}"
                        form-ref="home-filters" />
                    <x-button type="submit" class="px-4" aria-label="Submit search">
